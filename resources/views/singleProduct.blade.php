...
<script>
    ...
    // Build variant map for easy lookup
    ...
    function selectSize(size) {
        // Nếu click vào size đã được chọn, bỏ chọn nó
        if (selectedSize === size) {
            ...
            // Cập nhật lại color options dựa trên việc không có size được chọn
        ...
        // Check if we have both size and color selected
        updateSelectedVariant();
    }

    // Color selection function
    function selectColor(color) {
        // Nếu click vào color đã được chọn, bỏ chọn nó
        if (selectedColor === color) {
            selectedColor = null;
            selectedVariant = null;
            
            // Reset color button về trạng thái ban đầu
            ...
            // Cập nhật lại size options dựa trên việc không có color được chọn
    ...
    function updateColorOptions() {
        document.querySelectorAll('.color-option').forEach(btn => {
            ...
            const variant = variantMap[selectedSize] && variantMap[selectedSize][color];
            const hasStock = variant && variant.quantity > 0;
            
            if (hasStock) {
                btn.disabled = false;
                btn.classList.remove('border-gray-200', 'text-gray-400', 'cursor-not-allowed', 'bg-gray-50');
                btn.classList.add('border-gray-300', 'text-gray-700', 'cursor-pointer', 'hover:border-blue-500', 'hover:text-blue-600');
            } else {
                btn.disabled = true;
                btn.classList.remove('border-gray-300', 'text-gray-700', 'cursor-pointer', 'hover:border-blue-500', 'hover:text-blue-600', 'border-blue-500', 'text-blue-600', 'bg-blue-50');
                btn.classList.add('border-gray-200', 'text-gray-400', 'cursor-not-allowed', 'bg-gray-50');
            }
        });
    }

    // Update color options when size is selected
    ...
    // Update color options when no size is selected (back to original state)
    function updateColorOptionsWithoutSize() {
        document.querySelectorAll('.color-option').forEach(btn => {
            const color = btn.getAttribute('data-color');
            // Check if this color has any variants with stock
            const hasAnyStock = variants.filter(v => v.color === color && v.quantity > 0).length > 0;
    ...
    // Update size options when color is selected
    function updateSizeOptionsWithColor() {
        document.querySelectorAll('.size-option').forEach(btn => {
            const size = btn.getAttribute('data-size');
            const variant = variantMap[size] && variantMap[size][selectedColor];
            const hasStock = variant && variant.quantity > 0;
    ...
    // Update size options when no color is selected (back to original state)
    function updateSizeOptionsWithoutColor() {
        document.querySelectorAll('.size-option').forEach(btn => {
            const size = btn.getAttribute('data-size');
            // Check if this size has any variants with stock
            const hasAnyStock = variants.filter(v => v.size === size && v.quantity > 0).length > 0;
    ...
    // Reset color options to disabled state
    ...
    function updateSelectedVariant() {
        if (selectedSize && selectedColor) {
            selectedVariant = variantMap[selectedSize] && variantMap[selectedSize][selectedColor];
    ...
    function showVariantInfo() {
        ...
        detailsSpan.textContent = `Size ${selectedSize}, Màu ${selectedColor}`;
        stockSpan.textContent = selectedVariant.quantity;
        priceSpan.textContent = new Intl.NumberFormat('vi-VN').format(selectedVariant.price) + 'đ';
    ...
    function updateQuantityMax() {
        ...
        if (selectedVariant) {
            quantityInput.max = Math.min(selectedVariant.quantity, 10);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Add default styling to non-active buttons
